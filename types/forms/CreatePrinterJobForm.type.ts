...
const ProjectBriefPage: NextPageWithLayout = () => {
  ...
  const submitForm = async (values: CreateProjectBriefForm) => {
    const service = values.services.map(({ serviceId, extras, customFields }) => ({
      serviceId,
      extras,
      customFields,
    }))
  ...
  return (
    <>
      ...
      {activeService && activeService.extras.length > 0 && (
        <div className="h-fit w-86 rounded-xl bg-white p-5">
...
const Extras = ({
  ...
  const toggleExtras = ({ currentTarget: { checked } }: ChangeEvent<HTMLInputElement>) => {
    if (activeService) {
      ...
      if (checked) {
        if (service) {
          payload = [
            ...
            { ...service, extras: [...service.extras, { name: extrasName }] },
          ]
        } else {
          payload = [...services, { ...activeService, extras: [{ name: extrasName }] }]
        }
      ...
      if (serviceId === 14 && extrasName === 'Custom') {
        toggleCustomField()
      }
    ...
    <>
      <div className={`relative flex items-center ${disabled ? 'cursor-default opacity-40' : ''}`}>
        <input
          ...
          checked={
            !!services.find(
              ({ serviceId: sid, extras }) =>
                sid === serviceId && extras.includes({ name: extrasName })
            )
          }
        />
        ...
        {/* <div className="relative mt-5 flex w-40 items-center">
