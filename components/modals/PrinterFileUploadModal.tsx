...
export const usePrinterFileUploadModal = create(
  combine(
    {
      visible: false,
      files: [] as Array<File>,
    ...
    (set, get) => ({
      setVisible: (visible: boolean, printer?: PrinterJob) => set({ visible, printer }),
      addFiles: (
        files:
          | Array<File>
          | {
...
export const PrinterFileUploadModal = () => {
  const [tab, setTab] = useState<'upload' | 'my-files'>('upload')

  const { data: session } = useSession()
  const { showToast } = useToastStore()
  const queryClient = useQueryClient()
  const { printer, setVisible } = usePrinterFileUploadModal()
      ...
      {printer && (
        ...
        >
          ...
          {tab === 'upload' ? (
            ...
            <FileBrowserSelect clientId={printer.client.id || -1} />
          )}
