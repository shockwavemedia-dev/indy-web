const FloppyDiskIcon = ({ className }: { className?: string }) => (
  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path
      className={className}
      d="M13.5 5.70711V13C13.5 13.1326 13.4473 13.2598 13.3536 13.3536C13.2598 13.4473 13.1326 13.5 13 13.5H3C2.86739 13.5 2.74021 13.4473 2.64645 13.3536C2.55268 13.2598 2.5 13.1326 2.5 13V3C2.5 2.86739 2.55268 2.74021 2.64645 2.64645C2.74021 2.55268 2.86739 2.5 3 2.5H10.2929C10.3586 2.5 10.4236 2.51293 10.4842 2.53806C10.5449 2.56319 10.6 2.60002 10.6464 2.64645L13.3536 5.35355C13.4 5.39998 13.4368 5.4551 13.4619 5.51576C13.4871 5.57643 13.5 5.64145 13.5 5.70711Z"
      strokeWidth="1.25"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      className={className}
      d="M8.00049 11.5005C8.96699 11.5005 9.75049 10.717 9.75049 9.75049C9.75049 8.78399 8.96699 8.00049 8.00049 8.00049C7.03399 8.00049 6.25049 8.78399 6.25049 9.75049C6.25049 10.717 7.03399 11.5005 8.00049 11.5005Z"
      strokeWidth="1.25"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      className={className}
      d="M10 6.00049H6V2.50049"
      strokeWidth="1.25"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
  </svg>
)

export default FloppyDiskIcon
